...
app.put("/update/:beerId",async(req,res) => {
    const {accessToken,brewingName,ogValue,fgValue,alcohol} = req.body;
    const {beerId} = req.params;
    if(!accessToken || !brewingName || !ogValue || !fgValue || !alcohol || !beerId){
        return res.status(406).send({message:"Missing required fields.",check:false});
    }
    if(typeof accessToken !== "string" || typeof beerId !== "string" || typeof brewingName !== "string" || typeof ogValue !== "number" || typeof fgValue !== "number" || typeof alcohol !== "number"){
        return res.status(400).send({message:"Bad request, types of some fields are incorrect.",check:false});
    }
    if(accessToken === "" || beerId === "" || brewingName === "" || ogValue <= 0 || fgValue <= 0 || alcohol <= 0){
        return res.status(400).send({message:"Bad request, some fields are empty.",check:false});
    }
    const payload:JwtPayload|null = checkJwt(accessToken);
    if(!payload){
    ...
    const beer:Beer|null = await prisma.beer.update({
        where:{
            id:beerId
        },
        data:{
    ...
    return res.status(200).send({beer:beer,message:"Beer changed correctly.",check:true});
})
