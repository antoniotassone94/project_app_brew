...
import { Avatar, JwtKey, User } from "@prisma/client";
...
auth.post("/uploadavatar", async (req, res) => {
    const { accessToken, uploadavatar } = req.body;
    console.log(uploadavatar, accessToken)
    if (!accessToken || !uploadavatar) {
        return res.status(406).send({ message: "Missing required fields.", check: false });
    }
    if (uploadavatar != 'string') {
        return res.status(400).send({ message: "Bad request", check: false });
    }
    const payload: JwtPayload | null = checkJwt(accessToken);
    if (!payload) {
        return res.status(401).send({ message: "Token not valid", check: false });
    ...
    const avatar: Avatar | null = await prisma.avatar.create({
        data: {
            avatar: uploadavatar,
            userId: userId
        },
