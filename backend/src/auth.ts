...
auth.post("/deleteavatar",async(req,res) => {
    const {accessToken} = req.body;
    if(!accessToken){
        return res.status(406).send({message:"Missing required fields.",check:false});
    }
    ...
    if(user.avatar !== ""){
        const user1:User|null = await prisma.user.update({
            where:{
                id:userId
            },
            data:{
        ...
        return res.status(200).send({message:"Avatar deleted correctly.",check:true});
    }
